...
import axios from "axios";
import { 
  Trash2, UserPlus, Shield, Wallet, User, Lock, 
    ...
    <div className="min-h-screen bg-slate-950 p-8 text-slate-200">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">Add, remove, and manage system access and roles.</p>
        </div>
        
        {/* Create User Form */}
        <div className="bg-slate-900 p-6 rounded-xl shadow-lg border border-slate-800 mb-8">
          <h3 className="font-bold text-white mb-6 flex items-center gap-2 text-sm uppercase tracking-wider">
            <UserPlus size={16} className="text-blue-400"/> Onboard New Employee
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input 
                  placeholder="jdoe" 
                  className="w-full pl-9 p-2.5 bg-slate-950 border border-slate-700 rounded-lg text-sm text-white focus:ring-2 focus:ring-blue-500 outline-none transition" 
                  value={form.username}
                  onChange={e => setForm({...form, username: e.target.value})} 
                />
              </div>
            ...
            <div className="space-y-1">
              ...
              <input 
                placeholder="John Doe" 
                className="w-full p-2.5 bg-slate-950 border border-slate-700 rounded-lg text-sm text-white focus:ring-2 focus:ring-blue-500 outline-none transition" 
                value={form.name}
                onChange={e => setForm({...form, name: e.target.value})} 
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input 
                  type="number" 
                  className="w-full pl-7 p-2.5 bg-slate-950 border border-slate-700 rounded-lg text-sm text-white focus:ring-2 focus:ring-blue-500 outline-none transition" 
                  value={form.salary} 
                  onChange={e => setForm({...form, salary: +e.target.value})} 
                />
              </div>
            ...
            <button 
              onClick={handleCreate} 
              disabled={creating}
              className="bg-blue-600 text-white p-2.5 rounded-lg hover:bg-blue-500 font-bold text-sm transition shadow-lg shadow-blue-900/20 flex items-center justify-center gap-2 h-[42px]"
            >
              {creating ? <Loader2 className="animate-spin" size={18}/> : "Add User"}
            </button>
          </div>
        ...
        <div className="bg-slate-900 rounded-xl shadow-xl border border-slate-800 overflow-hidden">
          {loading ? (
            ...
            <div className="overflow-x-auto">
                ...
                <thead className="bg-slate-950 border-b border-slate-800 text-slate-400 uppercase text-xs">
                    <tr>
                    <th className="p-5 font-bold">Employee Details</th>
                    <th className="p-5 font-bold">Role Access</th>
                ...
                <tbody className="divide-y divide-slate-800">
                    ...
                    <tr key={u.username} className="hover:bg-slate-800/50 transition-colors group">
                        <td className="p-5 font-medium text-white">
                        <div className="flex items-center gap-3">
                            <div className="h-10 w-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-blue-400 font-bold text-sm shadow-inner">
                            {u.name.charAt(0)}
                            </div>
                            <div>
                        ...
                        {u.username !== 'admin' && (
                            <button 
                            onClick={() => handleDelete(u.username)} 
                            className="text-slate-500 hover:text-red-400 hover:bg-red-950/30 p-2 rounded-lg transition-all"
                            title="Delete User"
                            >
                            <Trash2 size={18}/>
                            </button>
